...
If you are using Let's Encrypt certificate, you'll need to [Manually Update Your Certificate]({{site.baseurl}}/certificates/#manually-update-a-lets-encrypt-certificate){:target="\_blank"}.
...
**A:** Before adding Bitwarden to system boot, complete [Docker Post-Installation]({{site.baseurl}}/install-on-premise/#docker-post-installation-linux-only) to setup a dedicated `bitwarden` service account.

...
This month, the Bitwarden team is pleased to release a set of community-driven features and updates. You asked, here's our answer:

- **Privacy & Security Options for Send**: Hide your email from Send recipients using a new Send Privacy option (see [here]({{site.baseurl/article/send-privacy/#hide-email}}) for details, and [here]({{site.baseurl}}/article/receive-send/#hidden-email-sends) for recommendations about receiving hidden-email Sends). Enterprise Organizations can implement a new Send Options policy to set the availability of this option (see [here]({{site.baseurl}}/article/policies/#send-options) for details).
- **FIDO Updates & Expanded Support**: FIDO implementation is upgraded from FIDO U2F to FIDO2 WebAuthn, but existing FIDO U2F keys will retain their integrity. FIDO Support has been expanded to more Browser Extensions and the Windows Desktop App (see [here]({{site.baseurl}}/article/setup-two-step-login-fido/) for details).
- **Disable Browser Extension Counter**: Disable the Browser Extension [Badge Counter]({{site.baseurl}}/article/auto-fill-browser/) using a new toggle in the {% icon fa-cogs %} **Settings** &rarr; **Options** menu (see [here]({{site.baseurl}}/article/auto-fill-browser/) for details).

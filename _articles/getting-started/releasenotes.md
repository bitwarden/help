...
- **Hide Email from Send**: Hide your email from Send recipients using a new Send Privacy option (see [here]({{site.baseurl/article/send-privacy/#hide-email}}) for details, and [here]({{site.baseurl}}/article/receive-send/#hidden-email-sends) for recommendations about receiving hidden-email Sends).

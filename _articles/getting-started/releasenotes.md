...
- **Disable Personal Vault Export Policy**: The Disable Personal Vault Export policy will prohibit all members of your Organization from exporting private Vault data (see [here]({{site.baseurl}}/article/policies/#disable-personal-vault-export) for details).
...
- **Browser Extension - Automatic Biometrics Prompt**: The Browser Extension will now automatically prompt for your biometric input when opened. You can disable this behavior from the {% icon fa-cogs %} **Settings** menu (see [here]({{site.baseurl}}/article/biometrics/) for details).
